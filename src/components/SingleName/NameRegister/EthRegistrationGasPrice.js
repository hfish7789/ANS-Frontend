...
const EthRegistrationGasPrice = ({ price, ethUsdPrice, gasPrice }) => {
  ...
  return (
    <PriceContainer>
      <TotalValue>
        <Typography sx={{ fontWeight: 400, mb: "16px" }}>{ethVal.toFixed(3)} AVAX + at most {registerGasFast.toFixed(3)} AVAX gas fee</Typography>
        <Paper sx={{ p: "10px 24px", background: "rgba(242, 97, 85, 0.05)", borderRadius: "16px" }}>
          <Stack direction='row' spacing={2}>
            <Typography sx={{ color: "#F26155", fontSize: "18px" }}>= {registerGasFast.toFixed(3)} AVAX</Typography>
            {ethVal && ethUsdPrice && (
              <USD>
                {' '}
